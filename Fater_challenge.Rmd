...
```{r}
stores_polygon <- st_read("C:/Users/giann/Downloads/shapes_NA.csv")
str(stores_polygon)

stores_data <- read.csv("C:/Users/giann/Downloads/stores_NA (2).csv")
str(stores_data)
```
...
socioDemo_storeMcode$PotenzialeQuartile <- cut(socioDemo_storeMcode$Potenziale, 
                                               ...
                                               labels = c("4° Quartile", "3° Quartile", "2° Quartile", "1° Quartile"),
                                               include.lowest = TRUE)
...
```{r}
kw_result <- kruskal.test(log(media_annuale) ~ fasciaoraria, data = so_st_gData)
print(kw_result)
kruskal.test(log(media_annuale) ~ daytype, data = so_st_gData)
...
This code generates an interactive bar plot that visualizes the average potential of the all the stores. By doing so, it offers insight into the variability of potential among different stores, allowing for a comparative analysis of store performance.
...
Determining the optimal number of clusters involves iterating over a range of cluster numbers (from 2 to 20) and computing silhouette scores for each clustering. The silhouette score measures the similarity of an object to its own cluster compared to other clusters. The number of clusters with the highest silhouette score is selected as the optimal number of clusters.

Finally, K-means clustering is performed using the previously determined optimal number of clusters. The resulting cluster assignments are stored in the cluster column of the merged_data dataframe.
...
This code generates an interactive bar plot showcasing the average store potential for each cluster. It enables users to visually explore the variations in store potential across different clusters.
...
Overall, the interactive bar plot facilitates insights into the average store potential distribution across various clusters identified through K-means clustering, thereby enabling interactive exploration and analysis of potential patterns.
...
# Filter the dataset to include only instances with parking available
socioDemo_storeMcode_with_parking <- socioDemo_storeMcode[socioDemo_storeMcode$Parking == "True", ]
 
# Select the variables you want to include in the clustering model
selected_vars <- c("Potenziale", "MQVEND")
 
# Standardize the selected variables
df_standardized <- scale(socioDemo_storeMcode_with_parking[, selected_vars])
...
for (i in 1:6) {
  # Fit k-means model with 'i' clusters
...
# Choose the number of clusters based on silhouette scores
...
# Assign cluster labels to the filtered dataset
socioDemo_storeMcode_with_parking$cluster <- kmeans_result$cluster
 
# Generate a frequency table of cluster labels
table(socioDemo_storeMcode_with_parking$cluster)
```


```{r}
```{r}
cluster_counts <- c( 67,  45,  76,  56,  10, 131  )
clusters <- 1:6
...
```{r}
average_potential <- socioDemo_storeMcode_with_parking %>%
  group_by(cluster) %>%
  summarise(AveragePotential = mean(Potenziale, na.rm = TRUE)) %>%
  arrange(desc(AveragePotential)) %>%
  mutate(Rank = rank(desc(AveragePotential)))

# Plotting the average potential by cluster using ggplot2
plot <- ggplot(average_potential, aes(x = factor(cluster), y = AveragePotential, fill = factor(cluster))) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste("Rank:", Rank), y = AveragePotential + 0.02), vjust = 0) +
  scale_fill_brewer(palette = "Set3") +
  labs(title = "Average Potential by Cluster", x = "Cluster", y = "Average Potential") +
  theme_minimal()

# Convert ggplot to plotly for an interactive plot
plotly_plot <- ggplotly(plot)

# If running this code in an interactive R session, you can display the plotly plot directly
plotly_plot
```
...
```{r}
set.seed(123)
socioDemo_storeMcode_with_parking <- socioDemo_storeMcode[socioDemo_storeMcode$Parking == "false", ]
socioDemo_storeMcode$store_type_num <- as.integer(as.factor(socioDemo_storeMcode$TipologiaPdV))
selected_vars <- c("Potenziale", "MQVEND","Parking")
...
#Average store potential by cluster

```{r}
df_standardized_n <- scale(socioDemo_storeMcode[, c("Potenziale")])
...
kmeans_result <- kmeans(df_standardized, centers = num_clusters)
socioDemo_storeMcode$cluster_potential <- kmeans_result$cluster
table(socioDemo_storeMcode$cluster_potential)
...
color_palette <- rainbow(length(unique(average_potential$cluster_potential)))

plotly_plot <- ggplotly(
  ggplot(average_potential, aes(x = cluster_potential, y = AveragePotential, fill = cluster_potential)) +
    geom_bar(stat = "identity", position = "dodge") +
    scale_fill_manual(values = color_palette) +  
    theme_minimal() +
    labs(title = "Average Store Potential by Cluster",
         x = "Store type cluster",
         y = "Average Potential")
)
...
result <- chisq.test(df_filtered$cluster, df_filtered$area_code)
print(result)

result <- chisq.test(df_filtered$cluster, df_filtered$population)
print(result)
```
Cluster vs. Potenziale:

The Chi-squared test comparing the variables cluster_potential and Potenziale resulted in a Chi-squared value of 9056.1 with 228 degrees of freedom. This suggests a significant association between the potential clusters identified and the clusters formed based on other criteria.

Cluster  vs. Store_type:

The Chi-squared test comparing cluster_potential and TipologiaPdV resulted in a Chi-squared value of 1979.6 with 36 degrees of freedom. This indicates a significant association between the potential clusters and the types of stores, suggesting that store potential varies across different store types.

Cluster  vs. Area Code:

The Chi-squared test comparing cluster_potential and area_code resulted in a Chi-squared value of 2357.9 with 1008 degrees of freedom. This implies a significant association between the potential clusters and the geographical areas represented by area codes, indicating regional variations in store potential.

The statistically significant results of the Pearson's Chi-squared tests provide strong evidence that the potential clusters identified exhibit significant associations with other variables such as store types and geographical areas. This suggests that store potential is not randomly distributed but is influenced by factors such as store type and geographic location. Further analysis and exploration of these associations can provide valuable insights for strategic decision-making in retail management and marketing efforts.
...
Model Training: A multiple local regression model is trained on the training dataset, with "Potenziale" as the dependent variable and "TipologiaPdV_cat", "Parking_cat", "MQVEND", and "area_code" as the independent variables.
...
socioDemo_storeMcode_with_parking$TipologiaPdV_cat = as.numeric(as.factor(socioDemo_storeMcode_with_parking$TipologiaPdV))
set.seed(123)
split = sample.split(socioDemo_storeMcode_with_parking$Potenziale, SplitRatio = 0.8)
training_set = subset(socioDemo_storeMcode_with_parking, split == TRUE)
test_set = subset(socioDemo_storeMcode_with_parking, split == FALSE)

loess_model <- loess(Potenziale ~ TipologiaPdV_cat + 
                       MQVEND +population ,span=1, data = test_set)
...
# Calcolo dell'MAE
mae <- mean(abs(test_set$Potenziale - predictions))
...
```
The R-squared value of 0.4416 for local regression indicates that approximately 44.16% of the variance in the response variable (Potenziale) can be explained by the predictors (MQVEND, TipologiaPdV_cat, Parking_cat, and area_code) included in the model.


```{r}
# Conversione della variabile 'TipologiaPdV' in numerica
socioDemo_storeMcode$TipologiaPdV_cat <- as.numeric(as.factor(socioDemo_storeMcode$TipologiaPdV))

# Conversione della variabile 'Parking' in numerica (0 per FALSE, 1 per TRUE)
socioDemo_storeMcode$Parking_cat <- as.numeric(as.factor(socioDemo_storeMcode$Parking))

# Assicurati che 'MQVEND' sia trattato come numerico se necessario
# socioDemo_storeMcode$MQVEND <- as.numeric(as.factor(socioDemo_storeMcode$MQVEND))

# Filtraggio per includere solo le righe dove Parking è FALSE (0 dopo la conversione in numerico)
socioDemo_storeMcode_filtered <- socioDemo_storeMcode[socioDemo_storeMcode$Parking_cat == 0, ]

# Divisione del dataset filtrato in training set e test set
set.seed(123)
split = sample.split(newdf4$Potenziale, SplitRatio = 0.8)
training_set = subset(newdf4, split == TRUE)
test_set = subset(newdf4, split == FALSE)

# Costruzione del modello loess sul training set
loess_model <- loess(Potenziale ~  MQVEND + population, span=1, data = training_set)
...
socioDemo_storeMcode_with_parking$TipologiaPdV_cat <- as.factor(socioDemo_storeMcode_with_parking$TipologiaPdV)
socioDemo_storeMcode_with_parking$Parking_cat <- as.numeric(as.factor(socioDemo_storeMcode_with_parking$Parking))

split <- sample.split(socioDemo_storeMcode_with_parking$Potenziale, SplitRatio = 0.8)
training_set <- subset(socioDemo_storeMcode_with_parking, split == TRUE)
test_set <- subset(socioDemo_storeMcode_with_parking, split == FALSE)
 
dt_model <- rpart(cluster ~  TipologiaPdV   + population, data = training_set, method = "class")
rpart_model <- rpart(cluster ~ population+TipologiaPdV, data = training_set, method = "class")
...
By interpreting the decision tree structure with of accuracy of 52.30 %, we can understand how the model makes predictions based on the values of the predictor variables. The root node is split based on the predictor variable that results in the best separation of the target variable cluster_potential.The splits in the tree determine the path an observation takes from the root node to a leaf node, where the final prediction is made based on the majority class of the observations in that leaf node.
...
df1<-socioDemo_storeMcode_with_parking%>%select(c("TipologiaPdV","cluster","population"))
str(df1)
df1$cluster<-factor(df$cluster)
df1$store_type_num<-factor(df$TipologiaPdV)
split = sample.split(df1$cluster, SplitRatio = 0.75)
training_set = subset(df1, split == TRUE)
test_set = subset(df1, split == FALSE)
 
library(randomForest)
rf_model <- randomForest(cluster ~ population+store_type_num, data = training_set,mtry=2, ntree = 1000)
...
# O specificamente per l'errore OOB
rf_model$err.rate
...
The Random Forest model achieved a relatively moderate accuracy of 44.21 %, indicating its effectiveness in predicting the 'cluster_potential' variable based on the given predictor variables.
The high accuracy suggests that the selected predictor variables ("TipologiaPdV_cat","area_code","cluster","cluster_potential", "Parking", "MQVEND" and "population") contain useful information for predicting the 'cluster_potential' categories.
...
# Imposta i parametri per evitare il pruning e mantenere tutte le variabili
control_params <- rpart.control(cp = 0, minsplit = 1, xval = 0,maxdepth =4)

# Addestra il modello di albero di decisione senza pruning
dt_model <- rpart(cluster ~ TipologiaPdV + Parking + population, 
                  data = training_set, 
                  method = "class",
                  control = control_params)

# Disegna l'albero di decisione mostrando tutte le variabili e le statistiche
rpart.plot(dt_model, type = 4, extra = 106)
predictions <- predict(rf_model, newdata = test_set)
confusion_matrix <- table(predictions, test_set$cluster)
